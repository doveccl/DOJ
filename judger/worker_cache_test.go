...
func TestDownloadProblemPackageUsesZipHashCache(t *testing.T) {
	...
	if err := downloadProblemPackage(t.Context(), server.Client(), cfg, 1000, "v1", testPackageFiles(), work1, 11, 11, 1); err != nil {
	...
	if _, err := os.Stat(filepath.Join(root, "cache", "P1000", "v1", "data", "1.in")); err != nil {
		t.Fatalf("problem cache data file missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(root, "cache", "P1000", "v1", "package")); !os.IsNotExist(err) {
	...
	if err := downloadProblemPackage(t.Context(), server.Client(), cfg, 1000, "v1", testPackageFiles(), work2, 12, 12, 1); err != nil {
...
func TestPackageFileListChangeReusesSameZip(t *testing.T) {
	...
	if err := downloadProblemPackage(t.Context(), server.Client(), cfg, 1000, "blob-1", []string{"data/1.in", "data/1.out"}, first, 1, 1, 1); err != nil {
		t.Fatal(err)
	}
	second := filepath.Join(root, "second")
	_ = os.MkdirAll(second, 0o755)
	if err := downloadProblemPackage(t.Context(), server.Client(), cfg, 1000, "blob-1", []string{"data/1.in"}, second, 2, 2, 1); err != nil {
		t.Fatal(err)
	}
	if downloads != 1 {
...
func TestDownloadProblemPackageSharesConcurrentDownload(t *testing.T) {
	...
	for index := 0; index < 2; index++ {
		go func(index int) {
			...
			errCh <- downloadProblemPackage(t.Context(), server.Client(), cfg, 1000, "v1", testPackageFiles(), work, uint(index+1), uint(index+1), 1)
		}(index)
	}
	<-entered
	close(release)
...
func TestDownloadProblemPackageReportsDownloadedBytes(t *testing.T) {
	...
	if err := downloadProblemPackage(t.Context(), server.Client(), cfg, 1000, "v1", testPackageFiles(), work, 11, 11, 1); err != nil {
...
func TestDownloadProblemPackageUsesLongerTimeoutThanAPIClient(t *testing.T) {
	...
	cfg := WorkerConfig{Server: server.URL, Cache: filepath.Join(root, "cache")}
	if err := downloadProblemPackage(t.Context(), client, cfg, 1000, "v1", testPackageFiles(), work, 11, 11, 1); err != nil {
