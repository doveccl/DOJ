...
- [PARTIAL] No compile cache yet, so B (submission package) still rebuilds per submission. Agent-side problem package/testdata cache is now implemented: worker includes file `sizeBytes` in `JudgeAgentPackageFile`, agent caches immutable S3 object bytes by `bucket/objectKey` with an LRU byte cap (`DOJ_AGENT_PACKAGE_CACHE_BYTES`, default 512MB), and `agent-cache` smoke covers hit/eviction/disabled-cache behavior.
