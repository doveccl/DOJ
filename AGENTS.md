...
- [DONE] OJ page-content research pass: Codeforces/AtCoder/DMOJ/Kattis/Luogu emphasize data and next actions, not marketing copy. Useful content patterns: home/news/announcements, active/upcoming/recent contests, new/recommended problems, recent submissions, ranking snippets, comment/discussion stream; problem list with filters plus solved/difficulty/AC stats; problem detail with limits/stats/tags and submit/submissions/discussion links; contest task table with per-task limits and standings; submission list with status/language/time/memory/score filters.
- [DONE] Home page no longer looks like a raw admin dashboard or marketing landing page: it now uses a compact practice overview with four actionable focus cards (recommended problem, contest status, assignment/discussion, latest submission), followed by problem/contest/discussion/submission data sections and admin-only stats below (`Home.vue`).
- [DONE] Problem detail now exposes OJ-standard metadata before the statement: time/memory limits, solved/submission counts, tags, and quick links to submissions/discussion; anonymous users see a compact submit prompt instead of a disabled full editor (`ProblemDetail.vue`).
