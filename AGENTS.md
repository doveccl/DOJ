...
- [DONE] Checker / special judge implemented. Optional C++ checker per problem version: agent compiles it once and invokes `checker <input> <output> <answer>` per case with testlib exit codes (0=AC, 2=PE, else WA), replacing exact/whitespace compare (`packages/runner/src/judge.ts`). Source flows from existing `checkerFileId` via the worker payload (`apps/worker/src/index.ts`), admin upload/edit endpoint `POST /api/problems/:id/checker` (`routes/problems.ts`) + AdminProblems modal. Covered by runner smoke. NOTE: **interactor still unimplemented** (two-way pipe; deferred follow-up).
- [DONE] Subtask / partial score implemented. `judgeCases` runs all cases (no first-error break) and awards each a weight (explicit per-case `points`, else even split of 100). Added `score` to `submissions`/`submission_cases` (baseline migration regenerated), `score`/`maxScore` in the agent protocol (`packages/shared/src/agent.ts`), persisted in worker, exposed on submission list/detail API, shown in submission UI. Covered by runner smoke (AC/RE/AC -> 67/100). NOTE: still single overall `status` (first failure's status) rather than a true subtask grouping model.
