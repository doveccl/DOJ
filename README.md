...
Judgers receive work through `/api/judger/lease` long polling. If no task is ready, the server holds the request briefly and returns `task: null`; when a submission changes, waiting leases wake and try to reserve work immediately. Task ownership and renewal live on the submission row (`judger_id`, `lease_until`, `attempt`). Session/cache state is source-exclusive: empty `REDIS` uses in-process memory, non-empty `REDIS` uses Redis.
...
评测机通过 `/api/judger/lease` 长轮询领取任务。没有任务时 server 会短暂挂起请求并返回 `task: null`；提交状态变化时，等待中的 lease 会被唤醒并立即尝试领取任务。任务归属和续租保存在提交记录的 `judger_id`、`lease_until`、`attempt`。session/cache 状态只认一个来源：`REDIS` 为空时使用进程内存，非空时使用 Redis。
