...
Judgers receive work through `/api/judger/lease` long polling. If no task is ready, the server holds the request briefly and returns `task: null`; when a submission changes, waiting leases wake and try to reserve work immediately. Task ownership and renewal live on the submission row (`judger_id`, `lease_until`, `attempt`); judger online state lives in Redis, with in-memory state only as a single-process fallback.
...
评测机通过 `/api/judger/lease` 长轮询领取任务。没有任务时 server 会短暂挂起请求并返回 `task: null`；提交状态变化时，等待中的 lease 会被唤醒并立即尝试领取任务。任务归属和续租保存在提交记录的 `judger_id`、`lease_until`、`attempt`；评测机在线态放 Redis，内存只作为单进程兜底。
