...
func TestSubmissionCanBeRejudgedByAdmin(t *testing.T) {
	...
	if err := judgeapi.SaveProgress(t.Context(), submission.ID, judgeapi.Progress{Attempt: 3, Stage: "judge", Done: 1, UpdatedAt: time.Now()}); err != nil {
	...
	progress, err := judgeapi.ReadProgress(t.Context(), submission.ID, 3)
	if err != nil {
