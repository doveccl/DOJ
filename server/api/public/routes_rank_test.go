...
func TestDatabaseRankUsesVisibleSubmissionStatsAndActiveUsers(t *testing.T) {
	...
	if !ok || aliceGuest.AC != 2 || aliceGuest.Submit != 3 {
		t.Fatalf("alice guest stats should exclude the running OI row: %+v", guestRank)
	}
	aliceProfile := decodeJSON[contract.UserProfile](t, requestOK(t, e, http.MethodGet, "/api/users/alice", ""))
	if aliceProfile.User.AC != 2 || aliceProfile.User.Submit != 3 {
		t.Fatalf("alice profile should not expose the running OI row: %+v", aliceProfile.User)
	}
...
func TestUserStatsUseViewerAwareSubmissionRows(t *testing.T) {
	...
	problems := make([]models.Problem, 4)
	...
	for _, row := range []any{&assignment, &oi, &icpc} {
	...
	for _, link := range []models.ContestProblem{{ContestID: oi.ID, ProblemID: problems[2].ID, Sort: "A"}, {ContestID: icpc.ID, ProblemID: problems[3].ID, Sort: "A"}} {
		if err := db.Create(&link).Error; err != nil {
			t.Fatalf("create contest problem: %v", err)
		}
	}
	assignmentID, oiID, icpcID := assignment.ID, oi.ID, icpc.ID
	submissions := []models.Submission{
		{UserID: student.ID, ProblemID: problems[0].ID, Language: "cpp", Code: "practice", Status: "AC", Score: 100, CreatedAt: now},
	...
	e := echo.New()
	Register(e, db)
	assertStats := func(label string, cookies []*http.Cookie, want, wantSolved int) {
		t.Helper()
		profile := decodeJSON[contract.UserProfile](t, requestWithCookies(e, http.MethodGet, "/api/users/student", cookies, nil))
		if profile.User.Submit != want || profile.User.AC != want || countForDate(profile.Heatmap, now.Format("2006-01-02")) != want {
			t.Fatalf("%s profile leaked hidden rows: user=%+v heatmap=%+v", label, profile.User, profile.Heatmap)
		}
		if profile.Solved.Total != int64(wantSolved) {
		...
		if !ok || got.Submit != want || got.AC != want {
			t.Fatalf("%s rank leaked hidden rows: %+v", label, rank)
		}
	}
	assertStats("guest", nil, 1, 1)
	assertStats("outsider", databaseSession(t, db, outsider.ID), 1, 1)
	assertStats("student", databaseSession(t, db, student.ID), 3, 2)
	assertStats("admin", databaseSession(t, db, admin.ID), 4, 4)
}
