...
func TestPrivateSubmissionSourceVisibilityWithDatabase(t *testing.T) {
	...
	if got := decodeJSON[contract.SubmissionDetail](t, requestWithCookies(e, http.MethodGet, privateTarget, databaseSession(t, db, admin.ID), nil)); got.Code != "secret" {
...
func TestSubmissionDetailIncludesTopLevelMessage(t *testing.T) {
	...
	got := decodeJSON[contract.SubmissionDetail](t, requestOK(t, e, http.MethodGet, target, ""))
...
func TestSubmissionDetailIncludesProgressButListDoesNot(t *testing.T) {
	...
	if err := judge.SaveProgress(t.Context(), submission.ID, judge.Progress{Attempt: 4, Stage: "judge", Done: 3, Total: &total, UpdatedAt: time.Now()}); err != nil {
...
func TestSubmissionPublicCanBeUpdatedByOwnerOrAdmin(t *testing.T) {
	...
	if got := decodeJSON[contract.CreatedID](t, ownerRes); got.ID != submission.ID {
	...
	if got := decodeJSON[contract.CreatedID](t, adminRes); got.ID != submission.ID {
...
func TestContextSubmissionSourceLockedUntilContextEnds(t *testing.T) {
	...
	if got := decodeJSON[contract.SubmissionDetail](t, requestWithCookies(e, http.MethodGet, target(submissions[0]), otherCookies, nil)); got.Code != "" || got.Submission.Status != "AC" {
	...
	if got := decodeJSON[contract.SubmissionDetail](t, requestWithCookies(e, http.MethodGet, target(submissions[2]), otherCookies, nil)); got.Code != "live assignment" || got.Submission.Status != "AC" {
		t.Fatalf("assignment should not gate public source or results: %+v", got)
	}
	...
	if got := decodeJSON[contract.SubmissionDetail](t, requestWithCookies(e, http.MethodGet, target(submissions[3]), otherCookies, nil)); got.Code != "ended assignment" {
		t.Fatalf("other user should read ended assignment public source: %+v", got)
	}
	if got := decodeJSON[contract.SubmissionDetail](t, requestWithCookies(e, http.MethodGet, target(submissions[4]), otherCookies, nil)); got.Code != "" {
...
func TestSubmissionEndpointsReadHiddenProblemSubmissionsButHideFields(t *testing.T) {
	...
	if list.Total != 1 || len(list.Items) != 1 || list.Items[0].ID != submission.ID {
		t.Fatalf("assignment submission should remain accessible: %+v", list)
	}
	if got := decodeJSON[contract.SubmissionDetail](t, requestWithCookies(e, http.MethodGet, "/api/submissions/"+strconv.FormatUint(uint64(submission.ID), 10), otherCookies, nil)); got.Code != submission.Code || got.Submission.Status != "AC" {
		t.Fatalf("assignment submission detail should use normal visibility: %+v", got)
	}
...
func TestRunningAssignmentShowsResultsButHidesOtherSource(t *testing.T) {
	...
	if res.Code != http.StatusOK {
		t.Fatalf("assignment should not gate submission detail, got %d body=%s", res.Code, res.Body.String())
	}
	got := decodeJSON[contract.SubmissionDetail](t, res)
	if got.Submission.Status != "WA" || got.Code != "homework" {
		t.Fatalf("assignment should not hide result or public source: %+v", got)
	}
}
