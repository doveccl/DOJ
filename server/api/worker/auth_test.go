...
func TestAuthAndLeaseAuthorization(t *testing.T) {
	...
	id, err := api.ensureJudger(local, judger.LeaseRequest{})
	if err != nil || id == 0 {
...
func TestLocalLoopbackConcurrentEnsureReusesJudgerWithoutToken(t *testing.T) {
	...
	for i := 0; i < workers; i++ {
		...
		go func() {
			defer wg.Done()
			_, err := api.ensureJudger(localJudgerContext(t, api), judger.LeaseRequest{Host: "local-judger"})
			errs <- err
		}()
	}
	wg.Wait()
...
func TestJudgerRequestNameDefault(t *testing.T) {
	if got := judgerRequestName(judger.LeaseRequest{Host: " linux-a "}); got != "linux-a" {
		t.Fatalf("name = %q", got)
	}
	if got := judgerRequestName(judger.LeaseRequest{}); got != "local-judger" {
